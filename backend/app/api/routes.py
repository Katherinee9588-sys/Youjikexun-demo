...
@router.get("/debug/asr-env")
def debug_asr_env() -> dict[str, bool]:
    """Temporary debug: show which ASR vars are present (bool only, no values)."""
    return {
        "ASR_API_KEY_set": bool((os.getenv("ASR_API_KEY") or "").strip()),
        "ASR_BASE_URL_set": bool((os.getenv("ASR_BASE_URL") or "").strip()),
        "ASR_MODEL_set": bool((os.getenv("ASR_MODEL") or "").strip()),
